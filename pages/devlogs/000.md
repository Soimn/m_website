...
**Disclaimer**: this section is highly messy and is more a brain dump than anything.
...
Although this also has some flaws, both of the implementations have a problem of being too general, which indicates that there should be a way of narrowing the accepted types for each parameter. Odin supports some kind of narrowing with type slicing and where clauses. Type slicing is a way of making aliases to polymorphic types, and specifying patterns they should match, while where clauses can be used to reject certain specializations by requiring a condition to be true.

```
//// type slicing
proc(t: $T/[]$E) // t can be any type T that is a slice of any type E
proc(t: $T/int)  // t can be any type T that is an int
proc(t: $T/$E/Frame_Local($S)) // t can be any type T, E that is a Frame_Local with the parameter of any type S


// where clause
proc(t: $T) where T == int // only type match against invocations where T is an int
```

Where clauses aid type slicing by allowing a set of typeids to pass, instead of every type, or just one (e.g. proc(t: $T) allows every type, proc(t: int) allows single, proc(t: $T) where T == int || T == f32 allows a closed set). I can't find any immediate weakness with type slicing, but where clauses, as they work in Odin, do have a drawback. The problem is that they only accept an expression, and since Odin does not support compile time execution, this cannot involve a regular procedure call. The solution to this is introducing compile time execution, and allowing where clauses to execute arbitrary code at compile time. This is kind of what Jai does, which brings a ton of expressive power to the table. However, it also introduces a whole lot of complexity in both the language and compiler. In this case compile time execution can be introduced whenever, and it won't affect the where clauses, only limit the expressiveness, which is why I will opt to see if this is something that is necessary before deciding to go with it.
 

Another topic dealing with types is variadic arguments. In C this is done rather poorly, with no type information available at all, but in Odin this is improved by adding the type 'any', which serves as a general runtime container with type information. This 'any' type allows writing type safe print procedures, at the expense of taking more space and introducing one layer of indirection. I my opinion, this approach is quite reasonable. However, what I don't necessarily agree with is the decision to bake this into the calling convention. Although writing procedure calls with variable arguments, like in C, is nice, it would be just as fine to add a bit of syntax, and remove the weirdness from the calling convention. What I would propose is something like the following:
...
Which seems like a weird change, since weird syntax is just replaced with some other weird syntax. However, this slight change allows the procedure to use the C calling convention, instead of a custom calling convention that specifies how the 'any' arguments are passed. This is all possible due to the inferred type of the array literal (.\[\]). This also easily allows several variadic argument lists, by turning them into arrays of type tagged pointers. I will opt to do it this way because I would like to keep the C calling convention for easy interop with C.


Finally, Odin has the concept of untyped types, which are types that exist to allow restricted implicit conversions. This is to allow the number 1 to implicitly convert to any numerical type, instead of having to tag every numeric literal with the intended type and casting. This is a concept that seems highly useful, although I am a bit put off by the name. I have decided to call these types 'soft types' instead, as apposed to regular typed (like int) which would be 'hard types'. The whole reason for doing this is to have a very strict regime on implicit conversions, so that overloading becomes less ambiguous. The question is then how complex these rules should be, since the soft type rules could be very simple, at the expense of convenience, or they could incorporate a lot of special cases to smooth this out. An example of such a special case is with operators vs. procedure calls (a + 1 could incorporate more implicit casts than c(1)), but this would not help if operators are replaced with procedure calls for operator overloading. This seems like a kind of lose lose situation, which indicates to me that I should probably reconsider how adding overloads should work. The problem is that, if the metaprogram can add overloads at any point, the compilation might never complete, since this could cause deadlocks due to unresolved dependencies due to implicit conversion ambiguities. One possible mitigation is to use explicit overloading, like in Odin, and allowing overloads to be added to this set. This would limit the dependency hell, but it would not solve the problem, and it would also limit the expressiveness of the language by not allowing addition of overloads besides editing the overload list. The best decision for the language would probably be to go with implicit overloading, and I will therefore opt to go with this and delay the decision on how to deal with adding overloads after experimenting more with metaprogramming.
...
Efficient usage of this requires nested struct declarations and the ability to access those nested structs. Both using and nested structs will be part of the language.


# Conclusion
That concludes what I would consider the foundation of the language. There are still constructs I mean to add, like modules or packages, but this will have to wait, as I am now going to implement this design and determine what needs to change. 